...
void MANDEL_mandel(const dacpp::Vector<complex<float> > & complex_points, dacpp::Vector<int> & mandelbrot_flags) { 
	...
	buffer<complex<float>, 1> b_complex_points(h_complex_points, range<1>(complex_points_Size));
    ...
    std::vector<int> init(mandelbrot_flags_Size, 0);
    sycl::buffer<int> r_mandelbrot_flags(init.data(), sycl::range<1>(mandelbrot_flags_Size));
    ...
    //结果返回
    mandelbrot_flags_tool.UpdateData(r_mandelbrot_flags,b_mandelbrot_flags,q,mandelbrot_flags_Size);
    {
