...
void LWR_shell_lwr(const dacpp::Vector<double> & rho, dacpp::Vector<double> & new_rho) { 
	...
	buffer<double, 1> b_rho(h_rho, range<1>(rho_Size));
    ...
    rho_tool.init(info_rho,rho_ops,q);
    ...
    std::vector<double> init(new_rho_Size, 0);
    sycl::buffer<double> r_new_rho(init.data(), sycl::range<1>(new_rho_Size));
    ...
    //结果返回
    new_rho_tool.UpdateData(r_new_rho,b_new_rho,q,new_rho_Size);
    {
