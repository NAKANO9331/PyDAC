...
void mdp_shell_mdp(const dacpp::Vector<double> & p, dacpp::Vector<double> & new_p) { 
    ...
    p.tensor2Array(h_p);
	buffer<double, 1> b_p(h_p, range<1>(p_Size));
    ...
    std::vector<double> init(new_p_Size, 0);
    sycl::buffer<double> r_new_p(init.data(), sycl::range<1>(new_p_Size));
    ...
    //结果返回
    new_p_tool.UpdateData(r_new_p,b_new_p,q,new_p_Size);
    {
