...
void PDE_pde(const dacpp::Vector<double> & u_kin, dacpp::Vector<double> & u_kout, const dacpp::Vector<double> & r) { 
	...
	buffer<double, 1> b_u_kin(h_u_kin, range<1>(u_kin_Size));
    ...
    r.tensor2Array(h_r);
	buffer<double, 1> b_r(h_r, range<1>(r_Size));
    ...
    std::vector<double> init(u_kout_Size, 0);
    sycl::buffer<double> r_u_kout(init.data(), sycl::range<1>(u_kout_Size));
    ...
    //结果返回
    u_kout_tool.UpdateData(r_u_kout,b_u_kout,q,u_kout_Size);
    {
