...
void ODDEVEN_oddeven(const dacpp::Vector<int> & array, dacpp::Vector<int> & array_out) { 
	...
	buffer<int, 1> b_array(h_array, range<1>(array_Size));
    ...
    array_tool.init(info_array,array_ops,q);
    ...
    std::vector<int> init(array_out_Size, 0);
    sycl::buffer<int> r_array_out(init.data(), sycl::range<1>(array_out_Size));
    ...
    //结果返回
    array_out_tool.UpdateData(r_array_out,b_array_out,q,array_out_Size);
    {
