...
void jacobiShell_jacobi(const dacpp::Matrix<float> & A, const dacpp::Vector<float> & b, const dacpp::Vector<float> & x, dacpp::Vector<float> & x_new, const dacpp::Vector<int> & nums) { 
    ...
    A.tensor2Array(h_A);
	buffer<float, 1> b_A(h_A, range<1>(A_Size));

    // 数据移动
    float* h_b = (float*)malloc(b_Size*sizeof(float));
    b.tensor2Array(h_b);
	buffer<float, 1> b_b(h_b, range<1>(b_Size));

    // 数据移动
    float* h_x = (float*)malloc(x_Size*sizeof(float));
    x.tensor2Array(h_x);
	buffer<float, 1> b_x(h_x, range<1>(x_Size));
	...
	buffer<int, 1> b_nums(h_nums, range<1>(nums_Size));
    ...
    std::vector<float> init(x_new_Size, 0);
    sycl::buffer<float> r_x_new(init.data(), sycl::range<1>(x_new_Size));
    ...
    nums_tool.init(info_nums,nums_ops,q);
    ...
    //结果返回
    x_new_tool.UpdateData(r_x_new,b_x_new,q,x_new_Size);
    {
