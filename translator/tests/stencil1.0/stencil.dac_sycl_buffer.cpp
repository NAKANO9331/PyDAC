...
void stencilShell_stencil(const dacpp::Matrix<double> & matIn, dacpp::Matrix<double> & matOut) { 
	...
	buffer<double, 1> b_matIn(h_matIn, range<1>(matIn_Size));
    ...
    matIn_tool.init(info_matIn,matIn_ops,q);
    ...
    std::vector<double> init(matOut_Size, 0);
    sycl::buffer<double> r_matOut(init.data(), sycl::range<1>(matOut_Size));
    ...
    //结果返回
    matOut_tool.UpdateData(r_matOut,b_matOut,q,matOut_Size);
    {
