...
void imageAdjustment_image_1(const dacpp::Matrix<Pixel> & image_tensor, dacpp::Matrix<Pixel> & image_tensor2) { 
    ...
    // Buffer设备内存分配
    buffer <Pixel> b_image_tensor2{image_tensor2_Size};
	...
	buffer<Pixel, 1> b_image_tensor(h_image_tensor, range<1>(image_tensor_Size));

    // 数据移动
    Pixel* h_image_tensor2 = (Pixel*)malloc(image_tensor2.getSize()*sizeof(Pixel));
    ...
    buffer<Pixel> r_image_tensor{image_tensor_Size};
    image_tensor_tool.Reconstruct(r_image_tensor,b_image_tensor,q);
    ...
    // buffer<Pixel> r_image_tensor2{image_tensor2_Size};
    std::vector<Pixel> init(image_tensor2_Size, Pixel{0, 0, 0});
    sycl::buffer<Pixel> r_image_tensor2(init.data(), sycl::range<1>(image_tensor2_Size));
    ...
    q.submit([&](handler &h) {
        ...
        accessor acc_image_tensor{r_image_tensor, h};
        accessor acc_image_tensor2{r_image_tensor2, h};
        ...
        h.parallel_for(sycl::nd_range<2>(global, local), [=](sycl::nd_item<2> item) {
            ...
            auto* d_image_tensor = acc_image_tensor.get_multi_ptr<access::decorated::no>().get();
            auto* d_image_tensor2 = acc_image_tensor2.get_multi_ptr<access::decorated::no>().get();
            // 嵌入计算
    ...
    //结果返回
    image_tensor2_tool.UpdateData(r_image_tensor2,b_image_tensor2,q,image_tensor2_Size);
    {
        host_accessor temp_accessor{b_image_tensor2};
        for(int i = 0; i < image_tensor2_Size; i++){
            h_image_tensor2[i] = temp_accessor[i];
        }
    }
