...
void waveEqShell_waveEq(const dacpp::Matrix<double> & matCur, const dacpp::Matrix<double> & matPrev, dacpp::Matrix<double> & matNext) { 
	...
	buffer<double, 1> b_matCur(h_matCur, range<1>(matCur_Size));

    // 数据移动
    double* h_matPrev = (double*)malloc(matPrev_Size*sizeof(double));
    matPrev.tensor2Array(h_matPrev);
	buffer<double, 1> b_matPrev(h_matPrev, range<1>(matPrev_Size));
    ...
    matCur_tool.init(info_matCur,matCur_ops,q);
    ...
    matPrev_tool.init(info_matPrev,matPrev_ops,q);
    ...
    std::vector<double> init(matNext_Size, 0);
    sycl::buffer<double> r_matNext(init.data(), sycl::range<1>(matNext_Size));
    ...
    //结果返回
    matNext_tool.UpdateData(r_matNext,b_matNext,q,matNext_Size);
    {
