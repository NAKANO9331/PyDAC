...
void DECAY_decay(const dacpp::Vector<double> & N0s, const dacpp::Vector<double> & lambdas, dacpp::Vector<double> & local_A, const dacpp::Vector<double> & t) { 
	...
	buffer<double, 1> b_N0s(h_N0s, range<1>(N0s_Size));

    // 数据移动
    double* h_lambdas = (double*)malloc(lambdas_Size*sizeof(double));
    lambdas.tensor2Array(h_lambdas);
	buffer<double, 1> b_lambdas(h_lambdas, range<1>(lambdas_Size));
    ...
    t.tensor2Array(h_t);
	buffer<double, 1> b_t(h_t, range<1>(t_Size));
    ...
    N0s_tool.init(info_N0s,N0s_ops,q);
    ...
    lambdas_tool.init(info_lambdas,lambdas_ops,q);
    ...
    std::vector<double> init(local_A_Size, 0);
    sycl::buffer<double> r_local_A(init.data(), sycl::range<1>(local_A_Size));
    ...
    //结果返回
    local_A_tool.UpdateData(r_local_A,b_local_A,q,local_A_Size);
    {
